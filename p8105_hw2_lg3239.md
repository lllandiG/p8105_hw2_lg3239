...
Using `prof_trash_wheel` on the `weight_tons` variable, and `sum()`, the
total weight of trash collected by Professor Trash Wheel is 190.12 tons.
Using `mr_trash_wheel`, `filter()` by 2020, on the `sports_balls`
variable, and `sum()`, the total number of sports balls collected by
...
``` r
pols_month_df = 
  read.csv("files/fivethirtyeight_datasets/pols-month.csv") %>%
  janitor::clean_names() %>%
